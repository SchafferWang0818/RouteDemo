...
//    NaviLatLng wayPoint3 = new NaviLatLng(39.965041, 116.447901);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        double start_lat = intent.getDoubleExtra("start_lat", 0.00);
        double start_lon = intent.getDoubleExtra("start_lon", 0.00);
        double end_lat = intent.getDoubleExtra("end_lat", 0.00);
        double end_lon = intent.getDoubleExtra("end_lon", 0.00);
        double[] pass_positions = intent.getDoubleArrayExtra("pass_position");
        sList.add(new NaviLatLng(start_lat,start_lon));
        eList.add(new NaviLatLng(end_lat,end_lon));
        for (int i = 0; i < pass_positions.length; i+=2) {
            if (pass_positions[i]!=0&&pass_positions[i+1]!=0){
                NaviLatLng naviLatLng = new NaviLatLng(pass_positions[i], pass_positions[i + 1]);
